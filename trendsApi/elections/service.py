...
class ElectionService:
    ...
    def process_and_save_word(cls, candidate, word, size, timeframe):
        ...
        words = word.split(' ') # Simplified as blacklist is missing
        if words and (diff_ratio < 0.85 or diff_slug_ratio < 0.85):
            ...
            if original:
                w = original.first()
                # w.status = True # status field was removed/missing in model, commenting out
                w.size += 5
                w.save()
            else:
                w = Word(candidate=candidate,
                         text=text,
                         # query_text=word, # query_text missing in model
                         size=size,
                         period=timeframe)
                w.save()
                print(word, size)
